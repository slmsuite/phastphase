...
def retrieve(far_field_intensities: jax.typing.ArrayLike,    #Magnitude squared of fourier transform
              support_shape: Tuple[int, int],       #Tuple of support extent in each direction
              support_mask: Optional[jax.typing.ArrayLike] = None,   #Optional support mask
    ...
    return refine(x_schwarz, far_field_intensities, support_shape, mask, winding_number, scale_gradient, max_iters, grad_tolerance)
...
def refine(x_init: jax.typing.ArrayLike,                 #Initial Guess for x
           ...
           support_shape: Tuple[int, int],                       #Tuple of support size in each direction 
           support_mask:Optional[jax.typing.ArrayLike] = None,   #Mask to indicate where x can be non-zero
    ...
    if scale_gradient:
        loss_scaling = 1/jnp.fmax(jnp.linalg.vector_norm(jax.grad(loss_func)(x0), ord=jnp.inf),1.0)
    else:
        loss_scaling=1
    def scaled_loss(x):
